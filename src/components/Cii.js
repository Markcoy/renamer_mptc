...
function Cii() {
  ...
  const formatDate = (date) => {
    ...
    return `${year}-${month}-${day}`; // Format: 2024-10-13
  };

  const getNewFileName = () => {
    if (file && selectedDate) {
      const formattedDate = formatDate(selectedDate);
      return `MPTC Group_Daily Traffic and Revenue Report_${formattedDate}.xlsx`;
    }
    return null;
  };
  ...
  return (
    ...
    >
      ...
      <h1 className="absolute text-2xl top-0 left-0 p-6  text-green-700">
        CII
      </h1>
      ...
      {file && selectedDate && (
        <>
          <p>
            New file name:{" "}
            <span className="text-green-700 text-xl">{getNewFileName()}</span>
          </p>
          <button
            onClick={handleDownload}
            className="mt-8 px-5 py-3 bg-green-500 text-white rounded-lg font-semibold text-md transition-all duration-200 ease-in-out transform hover:bg-green-600 focus:ring-4 focus:ring-blue-300 focus:outline-none active:scale-95"
          >
