...
function Etc_south() {
  ...
  const formatDate = (date) => {
    ...
    return dateObj.toISOString().split("T")[0]; // Format: 2024-10-13
  };

  const getNewFileName = () => {
    if (file && selectedDate) {
      const formattedDate = formatDate(selectedDate);
      return `MPTSOUTH_2024 RFID Penetration Rate and Per MOP Traffic_${formattedDate}.xlsx`;
    }
    return null;
  };
