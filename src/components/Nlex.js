...
function Nlex() {
  ...
  const formatDate = (date) => {
    ...
    return dateObj.toLocaleDateString("en-GB", options); // Format: 14 October 2024
  };

  const getNewFileName = () => {
    if (file && selectedDate) {
      const formattedDate = formatDate(selectedDate);
      return `MPT-NORTH_Daily Traffic and Revenue Report l ${formattedDate}.xlsx`;
    }
    return null;
  };
  ...
  return (
    <>
      <div
        ...
        id="nlex"
      >
        <h1 className="absolute text-2xl top-0 left-0 p-6 text-blue-700">
          MPT-NORTH/NLEX
        </h1>
